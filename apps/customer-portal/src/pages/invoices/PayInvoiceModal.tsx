...
export default function PayInvoiceModal({ isOpen, onClose, invoice, onPaid }: PayInvoiceModalProps) {
            ...
            >
                <div style={{
                    ...
                    <div style={{ color: '#fff', display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <div>
                            ...
                            <p style={{ color: '#BFDBFE', fontSize: 12, marginTop: 2 }}>{invoice.id}</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    {isPaid ? (
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, padding: '24px 0' }}>
                            ...
                            <div style={{ textAlign: 'center' }}>
                                <div style={{ fontSize: 18, fontWeight: 700, color: '#111827', marginBottom: 4 }}>Payment Successful!</div>
                                <div style={{ fontSize: 13, color: '#6B7280' }}>
                                    ${invoice.amount?.toLocaleString()} paid for {invoice.id}
                                </div>
                            </div>
                        </div>
                    ) : (
                        <>
                            ...
                            <div style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 12, padding: '16px 20px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div>
                                        ...
                                        <div style={{ fontSize: 24, fontWeight: 700, color: '#1D4ED8' }}>${invoice.amount?.toLocaleString()}</div>
                                    </div>
                            ...
                            <div>
                                ...
                                <div style={{ display: 'flex', gap: 10 }}>
                                    {[{ id: 'card' as PayMethod, label: '💳 Credit/Debit Card' }, { id: 'bank' as PayMethod, label: '🏦 Bank Transfer' }].map(m => (
                                        <button
                                            key={m.id}
                                            ...
                                            style={{
                                                ...
                                                fontSize: 13, fontWeight: payMethod === m.id ? 600 : 500,
                            ...
                            {payMethod === 'card' && (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                    <div>
                                        ...
                                        <input type="text" name="number" value={cardData.number} onChange={handleChange}
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="name" value={cardData.name} onChange={handleChange}
                                            placeholder="As shown on card" style={inputStyle} />
                                    </div>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                                        <div>
                                            ...
                                            <input type="text" name="expiry" value={cardData.expiry} onChange={handleChange}
                                                placeholder="MM / YY" maxLength={7} style={inputStyle} />
                                        </div>
                                        <div>
                            ...
                            {payMethod === 'bank' && (
                                <div style={{ background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: 10, padding: 18 }}>
                                    ...
                                    {[
                                        { label: 'Account Name', value: 'HVAC Pro Services Ltd' },
                                        { label: 'Account Number', value: '****  ****  1234' },
                                        { label: 'Sort Code', value: '12-34-56' },
                                        { label: 'Reference Number', value: invoice.id },
                                    ].map(row => (
                ...
                {!isPaid && (
                    <div style={{
                        ...
                        padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10,
                        flexShrink: 0,
                    }}>
                        <button
                            onClick={onClose}
                            style={{
                                padding: '10px 24px', borderRadius: 9, border: '1px solid #D1D5DB',
                        ...
                        <button
                            ...
                            style={{
                                ...
                                background: isLoading ? '#93C5FD' : '#2563EB', color: '#fff', fontSize: 13, fontWeight: 600,
                                cursor: isLoading ? 'not-allowed' : 'pointer',
                                display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'inherit',
                            }}
                        >
                            <CreditCard size={15} />
                            {isLoading ? 'Processing…' : `Pay $${invoice.amount?.toLocaleString()}`}
                        </button>
                    </div>
                )}
            </div>
